...
public class LeaderBoardDBActivity extends Activity {
	private final String TAG = "LightningRoundDBActivity";

	//** Added by Yashila on 9/18/2015 to replace xmlkiddatasource with DBDatasource
	private int mParticipantNumber = 1;
	private DojoDBAdapter mDbHelper;
	//** End changes
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//**Changed by Yashila on 9/18/2015 to replace xmlkiddatasource with DBDatasource
	...
	private void populateKidsTable(){
		//**Changed by Yashila on 9/18/2015 to replace xmlkiddatasource with DBDatasource
		Cursor c = mDbHelper.fetchAllParticipants();
		c.moveToFirst();

		for (int i=0; i<c.getCount(); i++){
			String name = c.getString(c.getColumnIndex(DojoDBAdapter.KEY_NAME));
	...
	public void onClickAdd(View view){
		try{
			...
			Boolean found = false;
			found = mDbHelper.findParticipant(newName);
			if (found){
